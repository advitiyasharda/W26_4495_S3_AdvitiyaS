...
def get_audit_log():
    ...
    try:
        ...
        # Query audit log database; map user_id -> friendly user name for frontend
        raw = get_db().get_audit_logs(limit=limit, offset=offset)
        audit_log = []
        for row in raw:
            # Prefer user_name from DB (joined from users), fallback to raw user_id or "system"
            user_name = row.get("user_name") or row.get("user_id") or "system"
            entry = {**row, "user": user_name}
