...
def test_recognition_with_webcam():
    ...
    frame_count = 0
    frames_with_face = 0      # frames where at least one face was detected
    frames_recognized = 0     # frames where at least one face was recognized (known person)
    ...
    while True:
        ...
        # Show statistics (rate = % of frames-with-face where we recognized someone)
        rate_pct = (100 * frames_recognized / frames_with_face) if frames_with_face > 0 else 0
        cv2.putText(frame, f'Frames: {frame_count} | Recognized: {frames_recognized}/{frames_with_face} ({rate_pct:.0f}%)',
        ...
        # ~30 FPS so it's not overwhelming; window must be focused for key press
    ...
    print(f"Frames with face detected: {frames_with_face}")
    print(f"Frames with recognized face: {frames_recognized}")
    if frames_with_face > 0:
        print(f"Recognition rate: {100*frames_recognized/frames_with_face:.1f}%")
